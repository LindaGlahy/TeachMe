import { getTeacherLessons, updateLessonState } from '../../js/models/lessonsModel.js';
...
async function loadTeacherLessons() {
    const lessons = getTeacherLessons(loggedUser.id);
            ...
            ${!isScheduled ? `
                ...
                <td class="text-center">
                    <div class="btn-group" role="group">
                        <button type="button" class="btn btn-success btn-sm accept-btn" data-id="${lesson.id}">
                            <i class="fas fa-check"></i>
                        </button>
                        <button type="button" class="btn btn-danger btn-sm reject-btn" data-id="${lesson.id}">
    ...
    if (!isScheduled) {
        container.querySelectorAll(".accept-btn").forEach(btn => {
            btn.addEventListener("click", function() {
                ...
                updateLessonStatus(lessonId, "accepted");
            });
        });
        ...
        container.querySelectorAll(".reject-btn").forEach(btn => {
            btn.addEventListener("click", function() {
                const lessonId = parseInt(this.dataset.id);
                updateLessonStatus(lessonId, "rejected");
            });
        });
    }
}
...
function updateLessonStatus(lessonId, status) {
    if (updateLessonState(lessonId, status)) {
        const message = status === "accepted" 
            ? "Aula aceita com sucesso!" 
            : "Aula rejeitada com sucesso!";
        showToast(message, status === "accepted");
        loadTeacherLessons();
    } else {
        showToast("Erro ao atualizar aula", false);
    }
}
